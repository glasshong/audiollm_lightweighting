...
class SALMONN_gemma2(nn.Module):
    @classmethod
    ...
    def __init__(
        ...
        logging.info('Loading Gemma2 Tokenizer')
        self.gemma_tokenizer = AutoTokenizer.from_pretrained(gemma_path, use_fast=False, token=token)
        self.gemma_tokenizer.add_special_tokens({'pad_token': '[PAD]'})
        self.gemma_tokenizer.padding_side = "right"

        if not only_preprocessor:
            logging.info('Loading Gemma2 Model')
            ...
            logging.info('Loading Gemma2 Done')
            ...
            if self.lora:
                ...
                self.gemma_model = get_peft_model(self.gemma_model, self.peft_config)
        ...
        if self.use_speech_Qformer:
            ...
            logging.info('Loading speech Gemma2 proj')
            if only_preprocessor:
                config = AutoConfig.from_pretrained(gemma_path, token=token)
                lm_hidden_size = config.hidden_size
            else:
                lm_hidden_size = self.gemma_model.config.hidden_size
            self.speech_gemma_proj = nn.Linear(
                self.speech_Qformer.config.hidden_size, lm_hidden_size
            )
            if speech_gemma_proj_model:
                logging.info("Loading speech Gemma2 proj from {}".format(speech_gemma_proj_model))
                speech_gemma_proj_weight = torch.load(speech_gemma_proj_model, map_location="cpu")
                self.load_state_dict(speech_gemma_proj_weight['model'], strict=False)
            if freeze_speech_gemma_proj:
                for name, param in self.speech_gemma_proj.named_parameters():
                    param.requires_grad = False
                self.speech_gemma_proj.eval()
                logging.info("freeze speech Gemma2 proj")
        else:
    ...
    def _encode_auditory_feature(self, speech_embeds, audio_embeds=None):
        with self.maybe_autocast():
            if self.use_speech_Qformer:
                ...
                speech_embeds = self.speech_gemma_proj(query_output.last_hidden_state)
    ...
    def prompt_wrap(self, embeds, atts, prompt, multi_prompt=False):
        if prompt:
            if multi_prompt:
                ...
                p_before_embeds = self.gemma_model.model.embed_tokens(p_before_tokens.input_ids) if not self.lora else self.gemma_model.model.model.embed_tokens(p_before_tokens.input_ids)
                ...
                p_after_embeds = self.gemma_model.model.embed_tokens(p_after_tokens.input_ids) if not self.lora else self.gemma_model.model.model.embed_tokens(p_after_tokens.input_ids)
                ...
                p_before_embeds = self.gemma_model.model.embed_tokens(p_before_tokens.input_ids).expand(batch_size, -1, -1) if not self.lora else self.gemma_model.model.model.embed_tokens(p_before_tokens.input_ids).expand(batch_size, -1, -1)
                p_after_embeds = self.gemma_model.model.embed_tokens(p_after_tokens.input_ids).expand(batch_size, -1, -1) if not self.lora else self.gemma_model.model.model.embed_tokens(p_after_tokens.input_ids).expand(batch_size, -1, -1)
    ...
    def forward(self, samples, verbose=False):
        ...
        to_regress_embeds = self.gemma_model.model.embed_tokens(to_regress_tokens.input_ids) if not self.lora else self.gemma_model.model.model.embed_tokens(to_regress_tokens.input_ids)
        ...
        ) * self.gemma_tokenizer.bos_token_id
        bos_embeds = self.gemma_model.model.embed_tokens(bos) if not self.lora else self.gemma_model.model.model.embed_tokens(bos)
        ...
        if verbose:
            nvocab = self.gemma_model.config.vocab_size
    ...
    def generate(self, samples, generate_cfg, prompts=None):
        ...
        text = self.gemma_tokenizer.batch_decode(outputs, add_special_tokens=False)

        return text

    @classmethod
    def from_config(cls, config):
        gemma_path = config.get("gemma_path")
        ...
        speech_gemma_proj_model = config.get("speech_gemma_proj_model", "")
        freeze_speech_gemma_proj = config.get("freeze_speech_gemma_proj", False)
        ...
        model = cls(
            ...
            speech_gemma_proj_model=speech_gemma_proj_model,
            freeze_speech_gemma_proj=freeze_speech_gemma_proj,
            lora=lora,
            lora_rank=lora_rank,
            lora_alpha=lora_alpha,
            lora_dropout=lora_dropout,
