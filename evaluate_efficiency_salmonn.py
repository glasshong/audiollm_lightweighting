...
def model_inference(cfg, samples, test_prompt, salmonn):
    ...
    bos = (
        ...
        * salmonn.gemma_tokenizer.bos_token_id
    )
    bos_embeds = (
    ...
    input_length = bos.shape[1] + speech_embeds.shape[1]
    cache_position = torch.tensor([input_length] * batch_size, device=speech_embeds.device)

    past_key_values = HybridCache(config=llm.config, max_batch_size=batch_size, max_cache_len=speech_embeds.shape[1], device=speech_embeds.device, dtype=torch.half)
    
    outputs = llm.model(
        inputs_embeds=speech_embeds.to(torch.float16),
    ...
    llm.config = Gemma2Config(sliding_window=4096)

    cache_kwargs = {
        "sliding_window": llm.config.sliding_window,
        "global_attention_span": 8192 , 
        "cache_position": cache_position
    }

    for layer_idx in range(llm.config.num_hidden_layers):
        key_states = outputs.past_key_values.key_cache[layer_idx][:, :, -1:, :].to(dtype=torch.half)
        value_states = outputs.past_key_values.value_cache[layer_idx][:, :, -1:, :].to(dtype=torch.half)

        past_key_values.update(key_states, value_states, layer_idx, cache_kwargs=cache_kwargs)
...
def main(args):
    ...
    gemma_model, _ = load_model(salmonn_preprocessor)
    salmonn_preprocessor.gemma_model = gemma_model
